...
def download_parallel(proj_id: str, bucket_name: str, st_uid: str,
    ...
    bl_names = []
    bl_lists = list_blobs(proj_id, bucket_name, prefix)
    for bl in bl_lists:
    ...
    for i, bl_name in enumerate(bl_names, 1):
        ...
        ps.queue.put(
            (
                download_blob, 
                (proj_id, bucket_name, bl_name, os.path.join(
...
study_uid = '1.3.6.1.4.1.14519.5.2.1.5382.4016.151028390933049970137573510138'
series_uid = '1.3.6.1.4.1.14519.5.2.1.5382.4016.316714047196016318134068344536'
log = []
log_ver = []
fix_: bool = True
...
for i in range(0, len(in_folders)):
    ...
    if download_:
        download_parallel(
            proj_id, bucket_name, study_uid, series_uid, in_folder, 88)
    ...
    if fix_:
        for i, ff in enumerate(in_files):
            ...
            FixFile(ff, os.path.join(fix_folder, base), fx_log, [], []) 
            # try:
    ...
    (v_file_pre, m_file_pre) = VER(fx_files[0], out_folder, log_ver)
    print(fx_files[0])
    for f in files:
        (v_file_pre, m_file_pre) = VER(f, out_folder, log_ver)
