...
def show_folder_content(folder, elapsed_time, heading:str = ''):
    ...
    speed_str = ct.get_human_readable_string(speed, True)+'Bps'
...
if os.path.exists(local_tmp_folder):
    shutil.rmtree(local_tmp_folder)
for thread_number in range(4, 64, 3):
    tic = time.time()
    ...
    tt['prallel'] = toc - tic
    show_folder_content(
        local_d_folder, toc - tic, 'parallel with: {}'.format(thread_number))
if os.path.exists(local_tmp_folder):
    shutil.rmtree(local_tmp_folder)
tic = time.time()
gs_util_cp(
    project_name, d_bucket_name, u_bucket_name, d_folder, local_d_folder)
toc = time.time()
tt['gs_util'] = toc - tic
