...
from rightdicom.dcmfix.fix_frequent_errors import(
    # FUNCTIONS
    fix_Trivials,
...
class ErrStatistics:
    ...
    def __init__(self):
        ...
        self.SOPClasses = set()
        self.Count = 0
# import rightdicom.dcmvfy.condn_cc
...
#------------------------------------------------------------------
    ...
    ctools.WriteStringToFile(os.path.join(
        f_rpt_folder, file_name +'_fix_report.txt'), fix_report)
...
def AddLogToStatistics(
        filename:str, vfilename:str, mfilename:str,
        log:list, stat:dict, regexp=''):
    ...
    for i in log:
        ...
        if i in stat:
            ...
            if filename not in stat[i].ErrorFiles:
                stat[i].ErrorFiles[filename] = FileUIDS(
                    filename, vfilename, mfilename)
...
def WriteFilesInfoSheet(wb:xlsxwriter.Workbook,
                        home_sheet, curr_sheet, error_files):
    ...
    for f, fuids in error_files.items():
        ...
        worksheet1.write(
            secondary_row, fs_1.index('SOP UID'), fuids.SOPInstanceUID)
        worksheet1.write(
            secondary_row, fs_1.index('STUDY UID'), fuids.StudyUID)
        worksheet1.write(
            secondary_row, fs_1.index('SERIES UID'), fuids.SeriesUID)
...
def WriteFixReportToWorksheet(seq_, excel_file):
    ...
    for err, obj in sorted(seq_.items(), key = lambda x:x[1].Count, reverse = True):
        ...
        file1_link = file1.replace(
            '/Users/afshin/Documents/work/VerifyDicom',
            git_url.format('tree/'+ commit))
        file2_name = os.path.basename(file2)
        file2_link = file2.replace(
            '/Users/afshin/Documents/work/VerifyDicom',
            git_url.format('tree/'+ commit))
        worksheet.write_url(r, fs.index('N'), 'internal:{}!A1'.format(
            r - row_offset), string = str(r-row_offset))
        ...
        worksheet.write_url(
            r, fs.index('CURRENT FUN FILE'), file1_link, string = file1_name)
        worksheet.write_string(r, fs.index('LAST FUN'), fun2)
        worksheet.write_url(
            r, fs.index('LAST FUN FILE'), file2_link, string = file2_name)
        WriteFilesInfoSheet(
            workbook, home_sheet, "{}".format(r-row_offset), obj.ErrorFiles)
...
def WriteVryReportToWorksheet(seq_, excel_file):
    ...
    commit = repo.head.object.hexsha
    worksheet.write_url(
        1, 1, git_url.format('commit/'+ commit), string = commit)
    worksheet.write_string(0, 2, 'LOCAL INPUT FOLDER' )
    worksheet.write_string(1, 2, in_folder)
    worksheet.write_string(0, 3, 'LOCAL OUTPUT FOLDER')
    worksheet.write_string(1, 3, out_folder)
    ...
    for err, obj in sorted(seq_.items(), key = lambda x:x[1].Count, reverse = True):
        worksheet.write_url(
            r, fs.index('N'), 'internal:{}!A1'.format(r - row_offset),
            string = str(r-row_offset))
        worksheet.write_number(r, fs.index('FREQ(FILES)'), obj.Count)
        worksheet.write_number(r, fs.index('FREQ(FOLDER)'), len(obj.ErrorDirs))
        worksheet.write_string(r, fs.index('ERROR'), err)
...
def FIX(in_folder, out_folder, prefix=''):
    ...
    if not os.path.exists(in_folder):
        return(
            pre_fix_error_statistics, post_fix_error_statistics,
    ...
    for i, f in enumerate(file_list, 1):
        ...
        time_elapsed_since_last_record = \
            time_point - last_time_point_record_data
        log_david_post = []
        log_david_pre = []
        log_fixed = []
        (v_pre, m_pre, v_post, m_post) = FixFile(
            f, in_folder, dcm_folder, fix_folder, pre_vfy_folder,
        ...
        AddLogToStatistics(
            fixed_file_path, v_post, m_post, log_david_post,
            post_fix_error_statistics,'Error.*')
        AddLogToStatistics(
            f, v_pre, m_pre, log_david_pre, pre_fix_error_statistics,'Error.*')
        AddLogToStatistics(
            fixed_file_path, v_post, m_post, log_david_post,
            post_fix_warning_statistics,'Warning.*')
        AddLogToStatistics(
            f, v_pre, m_pre, log_david_pre,
            pre_fix_warning_statistics,'Warning.*')
        ...
        if time_elapsed_since_last_record > time_interval_record_data:
            ...
            WriteFixReportToWorksheet(
                fix_rep, out_folder + fix_report_file_name +".xlsx")
            WriteVryReportToWorksheet(
                post_fix_error_statistics,
                out_folder + post_fix_error_file_name +".xlsx")
            WriteVryReportToWorksheet(
                pre_fix_error_statistics,
                out_folder + pre_fix_error_file_name +".xlsx")
            WriteVryReportToWorksheet(
                post_fix_warning_statistics,
                out_folder + post_fix_warning_file_name +".xlsx")
            WriteVryReportToWorksheet(
                pre_fix_warning_statistics,
                out_folder + pre_fix_warning_file_name +".xlsx")
